...
import jp.fieldnotes.hatunatu.util.misc.Base64Util;
...
public class XlsReader implements DataReader, DataSetConstants {
     ...
     *
     * @param path       パス
     * @param trimString 文字列をトリムするかどうか
     */
    ...
    protected void setupColumns(DataTable table, HSSFSheet sheet) {
        ...
        for (int i = 0; i <= Short.MAX_VALUE; ++i) {
            HSSFCell nameCell = nameRow.getCell(i);
            if (nameCell == null) {
                break;
            }
            ...
            if (valueRow != null) {
                for (int j = 1; j <= sheet.getLastRowNum(); j++) {
                    valueCell = sheet.getRow(j).getCell(i);
                    if (valueCell != null
     ...
     * 整数かどうかを返します。
     *
     * @param numericCellValue numericな値
     * @return 整数かどうか
     */
    ...
    private static FileInputStream create(File file) {
        try {
